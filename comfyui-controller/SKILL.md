...
3. [4.6.4 K 采样器选择](#464-k-采样器选择强制按提示词内容与目标效果) — **按提示词选采样器（强制）**
...
3. [4.6.4 K 采样器选择](#464-k-采样器选择强制按提示词内容与目标效果) — 视频采样器选择（dpm++_sde 等）
...
| 采样器怎么选（DPM++/DDPM）？    | dpmpp\_ddpm\_samplers\_guide.md                      | 第2/3章 各采样器详解+快速选择                                                             |
| K 采样器按提示词怎么选？         | SKILL.md 4.6.4 节                                     | 收敛型vs随机型 + 决策表 + 家族详解 + 调度器协同                                              |
...
| `KSampler`               | 采样去噪      | `seed`, `steps`, `cfg`, `sampler_name`, `scheduler`（**sampler_name 必须按 [4.6.4 节](#464-k-采样器选择强制按提示词内容与目标效果) 根据提示词选择，不可默认**） |
...
> **⚠️ 强制**：`sampler_name`（K 采样器）必须显式选择，根据提示词内容与目标效果决定，不可跳过或沿用默认值。完整选择方法论见 [4.6.4 节](#464-k-采样器选择强制按提示词内容与目标效果)。
...
#### 4.6.4 K 采样器选择（强制，按提示词内容与目标效果）

> **强制规则**：构建任何图片/视频工作流时，必须为 `sampler_name` 显式选择采样器，**不可跳过或沿用默认值**。选择依据 = 提示词内容所表达的画面目标（写实/创意/极速/稳定）+ 模型系列 + 硬件档位。以下为 ComfyUI 全部 K 采样器的选择方法论。

**第 1 步：判定画面目标 → 采样器类型（收敛型 vs 随机型）**

| 画面目标（从提示词判断） | 采样器类型 | 特性 |
|------|-----------|------|
| 产品图、建筑效果图、商业工作流、需要可复现 | **收敛型** | 确定性高，同种子可复现，结构扎实 |
| 概念艺术、风格化插画、角色设计、创意探索 | **随机型** | 每步注入噪声，纹理丰富，不收敛 |
...
| 采样器 | 类型 | 特点 | 最佳场景 |
|--------|------|------|---------|
| `euler` | 收敛 | 最简单，干净偏朴素，速度快 | 快速草稿、图标/Logo、低重绘图生图(denoise<0.4) |
| `euler_ancestral` | 随机 | 水彩/印象派纹理，不收敛 | 艺术风格化、动漫/插画（吉卜力/新海诚风） |
| `euler_cfg_pp` | 收敛 | 保留速度+提升提示词控制 | 快速出图且需控制力 |
| `euler_ancestral_cfg_pp` | 随机 | 创意与控制平衡 | 风格化创作需语义控制 |

**Heun 系列（二阶精度进阶）**：
| 采样器 | 类型 | 特点 | 最佳场景 |
|--------|------|------|---------|
| `heun` | 收敛 | RK2 两步法，更锐利、伪影少 | 写实人像、风景摄影 |
| `heunpp2` | 收敛 | Heun 质量天花板，计算贵 | 静物/珠宝/产品特写 |

**DPM/DPM++ 系列（现代主力，详细见 docs_cli/dpmpp_ddpm_samplers_guide.md）**：
| 采样器 | 类型 | 特点 | 最佳场景 |
|--------|------|------|---------|
| `dpmpp_2m` ★ | 收敛 | 速度质量完美平衡，行业黄金标准 | **日常通用首选** |
| `dpmpp_2m_cfg_pp` | 收敛 | 提示词遵循度高 | 复杂长提示词、商业约稿 |
| `dpmpp_2m_sde` | 随机 | 平滑自然，"胶片感" | 写实人像特写、产品摄影 |
| `dpmpp_2m_sde_heun` | 随机 | Heun 求解器更高精度 | 极致细节、最终出图 |
| `dpmpp_3m_sde` | 随机 | 理论最高保真度 | 大幅面打印、科学级图像、复杂纹理 |
| `dpmpp_2s_ancestral` | 随机 | 纹理丰富，意外惊喜 | 概念艺术、风格探索 |
| `dpmpp_sde` | 随机 | 平滑渐变、空气感/景深 | 写实、柔和过渡（显存占用较高） |
| `dpm_fast` | - | 极速，少函数评估 | 低分辨率预览 |
| `dpm_adaptive` | - | 自适应步长，质量极致 | 不计成本高质量（耗时不可预测） |

**经典与专业系列**：
| 采样器 | 类型 | 特点 | 最佳场景 |
|--------|------|------|---------|
| `ddim` | 收敛 | 干净稳定，色彩饱和偏高，构图保持强 | 建筑线稿上色、绝对稳定、老旧工作流兼容 |
| `uni_pc` | 收敛 | 统一预测-校正，效率高 | 追求速度不牺牲太多质量 |
| `uni_pc_bh2` | 收敛 | UniPC 二阶修正，SDXL 兼容好 | SDXL 模型、dpmpp_2m 备选 |
| `lms` | 收敛 | 线性多步法，老而稳 | 老旧工作流兼容（不推荐新项目） |
| `deis` | 收敛 | 指数积分，较新 | 技术尝鲜、dpmpp_2m 实验替代 |
| `ipndm`/`ipndm_v` | - | 实验性，文档少 | 不建议生产 |
| `ddpm` | 收敛 | 原始扩散，极慢 | 学术研究/对比实验（100-1000步） |

**特殊系列**：
| 采样器 | 类型 | 特点 | 最佳场景 |
|--------|------|------|---------|
| `res_multistep` 系列 | 收敛 | 分段优化策略，快 | NVIDIA Cosmos/MiniMax H3 特定模型、批量测试 |
| `seeds_2`/`seeds_3` | - | 内部/实验性 | 仅开发者测试 |

**`lcm`（潜在一致性模型，非 K 采样器家族）**：
| 采样器 | 特点 | 最佳场景 |
|--------|------|---------|
| `lcm` | **必须配 LCM LoRA/专用模型**，2-8 步直达结果，蒸馏风格 | 实时预览、视频流、移动端、极速交互（4-8步 + lcm调度器） |

**第 4 步：调度器协同（scheduler）**
...
**模型系列特殊约定（优先级高于上述通用规则）**：
- **Wan2.2 视频**：`dpm++_sde`（+lightx2v 验证）；unipc 导致动作卡住旋转（见 4.3.6 节）
- **Flux 1**：`euler` / `heun`（见 4.4.2 节）
- **Flux 2**：`euler` + `simple`，cfg=1（见 4.4.2 节）
- **SD1.5/SDXL**：`euler_ancestral` / `dpmpp_2m`（见 4.4.1 节）

**注**：详细 DPM++/DDPM 系列机制与梯度见 [docs_cli/dpmpp_ddpm_samplers_guide.md](docs_cli/dpmpp_ddpm_samplers_guide.md)；调度器场景化分析见 EXPERIENCE.md 21.8 节。
