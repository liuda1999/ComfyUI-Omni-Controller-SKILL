...
3. [4.19 K 采样器完全参考手册](#419-k-采样器完全参考手册comfyui-全家族) — **按提示词选采样器（强制，含全部类别）**
4. [6.3 关键 API 端点](#63-关键-api-端点) — `/object_info` 查询模型名

**你要生成视频？**
1. [4.2 视频生成通用流程](#42-视频生成通用流程适用于所有视频模型) — 7 步标准流程
2. [4.3 参考案例：Wan2.2](#43-参考案例wan22-视频生成v18v19-验证架构) — 节点链 + 参数梯度
3. [4.19 K 采样器完全参考手册](#419-k-采样器完全参考手册comfyui-全家族) — 采样器选择（4.19.6 模型系列参考）
4. [4.15 SVI Pro 长视频](#415-svi-pro-长视频工作流架构分析基于源码研读与-v12-实战验证) — 分段生成架构
5. [4.16 Flux2 修正工作流](#416-flux2-图片修正工作流img2img-模式) — 画质修正链路
6. [4.18 长视频色调/动作/拼接修复](#418-长视频色调动作拼接修复经验2026-07-29-验证) — 三大问题修复方案
7. [4.18.6 提示词工程完整方法论](#4186-提示词工程完整方法论引用) — 公式/人物/场景/动作/镜头/关系
8. [4.11 关键设计禁忌](#411-关键设计禁忌) — 踩坑前先看
...
1. [scripts/build\_workflow\_library.py](scripts/build_workflow_library.py) — 构建/更新工作流库
2. [scripts/query\_library.py](scripts/query_library.py) — 6 种维度查询
...
docs_cli/ 技术文档（统一入口 → docs_cli/INDEX.md 快速索引）:
      ...
      （采样器参考已整合进本文件 4.19 节 K 采样器完全参考手册）
```
...
| 你想知道...                | 查阅文件                                                 | 章节                                                                            |
| ---------------------- | ---------------------------------------------------- | ----------------------------------------------------------------------------- |
| 全量经验/技术文档怎么快速检索？       | docs\_cli/INDEX.md                                   | 主题速查（关键词→章节+行号）+ EXPERIENCE.md 全章节行号导航                                        |
| 某参数选什么值？               | EXPERIENCE.md                                        | 第21章 参数梯度分析（15大参数+综合矩阵）                                                       |
| 采样器怎么选（DPM++/DDPM）？    | SKILL.md 4.19.4 节                                    | DPM/DPM++ 全系列详解（含 ddpm）                                                     |
| K 采样器按提示词怎么选？         | SKILL.md 4.19 节（独立章节）                         | 全家族（Euler/Heun/DPM++/经典专业/res_multistep/LCM/DDPM）+ 决策表 + 模型系列参考 |
| Wan2.2 双卡/双实例怎么配？      | wan22\_dual\_instance\_tuning\_guide.md              | 0-3章 架构/阶段节点/标定/编排                                                            |
...
| 视频拼接有渐变？               | EXPERIENCE.md                                        | **第24章 overlap\_mode梯度分析** + SKILL.md 4.18.3节 + 8.8.3节                        |
...
| KSamplerAdvanced参数怎么填？ | EXPERIENCE.md                                        | 第19章 widgets\_values 10值对齐 + 第22章详细陷阱表                                        |
...
│   （采样器参考已整合进 SKILL.md 4.19 节，不再单独成文档）
...
#### 3.1.3 run\_workflow\.py - 工作流执行引擎
...
- **输出识别**：脚本默认解析 `images` 字段；视频任务输出在 `gifs` 字段（VHS\_VideoCombine 节点特性），需额外查询 `/history/{prompt_id}` 确认

- **错误处理**：通过 `/history` 的 `status.messages` 提取 `execution_error`，返回节点 ID、类型、异常信息

#### 3.1.4 check\_status.py - 环境检查器
...
#### 3.1.5 workflow\_converter.py - 格式转换器
...
#### 3.1.6 edit\_workflow\.py - 工作流编辑器
...
#### 3.1.7 dependency\_manager.py - 依赖管理器
...
#### 3.1.8 download\_models.py - 模型下载器
...
#### 3.1.9 get\_available\_models.py - 模型查询器
...
| `KSampler`               | 采样去噪      | `seed`, `steps`, `cfg`, `sampler_name`, `scheduler`（**sampler_name 必须按 [4.19 节](#419-k-采样器完全参考手册comfyui-全家族) 根据提示词选择，不可默认**） |
...
| ------------ | --------------------------- | ----------------------- | ------------------- | ------------------ | ----------------- |
| Wan2.2       | ComfyUI-WanVideoWrapper     | WanVideoModelLoader     | WanVideoSampler     | WanVideoDecode     | VHS\_VideoCombine |
| HunyuanVideo | ComfyUI-HunyuanVideoWrapper | HunyuanVideoModelLoader | HunyuanVideoSampler | HunyuanVideoDecode | VHS\_VideoCombine |
| CogVideoX    | ComfyUI-CogVideoXWrapper    | CogVideoModelLoader     | CogVideoSampler     | CogVideoDecode     | VHS\_VideoCombine |
| LTX-Video    | ComfyUI-LTXVideo            | LTXModelLoader          | LTXSampler          | LTXDecode          | VHS\_VideoCombine |
...
4. **VAE 解码禁用 tiling（视频任务）**：enable\_vae\_tiling 会导致视频帧间不一致
5. **VHS\_VideoCombine 高质量编码**：`crf=14` + `pix_fmt=yuv420p10le`
...
- 直接将 block\_swap\_args 传给 ModelLoader 的输入参数是错误的
...
| 节点                           | 功能                  | 备注                                    |
| ---------------------------- | ------------------- | ------------------------------------- |
| `WanVideoModelLoader`        | 加载 Wan 视频模型         | 不直接接收 block\_swap\_args 和 lora        |
| `WanVideoBlockSwap`          | 生成 BlockSwap 配置参数   | 独立节点，输出 BLOCKSWAPARGS                 |
| `WanVideoSetBlockSwap`       | 将 BlockSwap 配置应用到模型 | 接收 model + block\_swap\_args，输出 model |
| `WanVideoLoraSelect`         | 选择 LoRA 并设置强度       | 输出 WANVIDLORA                         |
| `WanVideoSetLoRAs`           | 将 LoRA 应用到模型        | 接收 model + lora，输出 model              |
| `WanVideoVAELoader`          | 加载 Wan VAE          | <br />                                |
| `LoadWanVideoT5TextEncoder`  | 加载 T5 文本编码器         | <br />                                |
| `WanVideoTextEncode`         | T5 编码提示词            | <br />                                |
| `WanVideoClipVisionEncode`   | CLIP Vision 编码图像    | <br />                                |
| `WanVideoImageToVideoEncode` | I2V 编码              | 输出 image\_embeds                      |
| `WanVideoSampler`            | Wan 专用采样器           | 支持双阶段串行                               |
| `WanVideoDecode`             | 解码视频潜在空间            | 禁用 enable\_vae\_tiling                |
| `INTConstant`                | 共享参数节点              | 用于 steps 和 split\_step                |
| `CreateCFGScheduleFloatList` | 动态 CFG 调度           | V18 关键改进                              |

#### 4.3.2 双阶段串行架构（Wan2.2 参考）

通过 WanVideoSampler 的 start\_step/end\_step 参数控制双阶段采样：

- HIGH 阶段: start\_step=0, end\_step=split\_step（处理高噪声主结构）

- LOW 阶段: start\_step=split\_step, end\_step=-1, samples=HIGH输出, add\_noise\_to\_samples=False（处理低噪声细化）

**动态 CFG 调度（Wan2.2 + lightx2v 验证）**：

- 使用 CreateCFGScheduleFloatList 节点生成动态 CFG 调度

- 参考配置: cfg\_scale\_start=2, cfg\_scale\_end=2, start\_percent=0.0, end\_percent=0.01
...
| ---------------- | ----- | ------------------- | ------------------- | ------------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `base_precision` | 基础精度  | bf16                | bf16                | bf16                | fp16\_fast     | fp16\_fast 在低显存+lightx2v 时 OOM                                                                                                                                                      |
| `quantization`   | 量化模式  | fp8\_e4m3fn\_scaled | fp8\_e4m3fn\_scaled | fp8\_e4m3fn\_scaled | bf16/fp8       | 按显存选择                                                                                                                                                                               |
| `load_device`    | 加载设备  | offload\_device     | offload\_device     | offload\_device     | default        | 显存紧张时必须 offload                                                                                                                                                                     |
...
| `rope_function`     | RoPE 函数     | comfy\_chunked | comfy\_chunked | comfy\_chunked | comfy\_chunked | 480x848 及以上必须使用                             |
...
| Q8\_0 GGUF    | `.gguf`            | 10-12GB 显存 | 低    | 接近 fp16 |
| Q5\_K\_M GGUF | `.gguf`            | 8-10GB 显存  | 很低   | 可接受     |
| Q4\_K\_M GGUF | `.gguf`            | <8GB 显存    | 最低   | 一般      |
...
| `clip`         | 文本编码器   | qwen\_3\_8b 系列        | type=flux2（必填）                  |
...
| 节点                      | 功能    | 关键差异                                 |
| ----------------------- | ----- | ------------------------------------ |
| `UNETLoader`            | 模型加载  | weight\_dtype=default（无 type 参数）     |
...
**备注**：Flux 1 与 Flux 2 架构完全不同，禁止混用。F2K-9b-kleinova 的 txt\_in 层期望 12288 维（Qwen3-8B 取第9/18/27层拼接=3×4096），T5XXL 只输出 4096 维，混用会导致 mat1/mat2 shapes cannot be multiplied 错误。
...
| 档位      | VRAM范围  | 推荐分辨率    | 单次最大帧数 | blocks\_to\_swap | base\_precision |
| ------- | ------- | -------- | ------ | ---------------- | --------------- |
| L1 入门级  | 8-12GB  | 352×640  | 81帧    | 40-42            | bf16            |
| L2 标准级  | 12-16GB | 480×640  | 121帧   | 38-40            | bf16            |
| L3 高性能级 | 16-24GB | 480×848  | 121帧   | 20-24（C8验证）      | bf16            |
| L4 专业级  | ≥24GB   | 576×1024 | 121帧   | 20-24            | fp16\_fast      |

> **C8 验证更新**：L3 档 blocks\_to\_swap 从 36-38 修正为 20-24。值过高会导致专用显存闲置（仅用 40%），转而使用共享 GPU 内存。降至 20 后专用显存利用率提升至 75%+。
...
4. **多模型加载策略（基于原 SVI Pro 工作流分析）**：
   ...
   - **同一模型文件场景**：原 SVI Pro 工作流中 high\_model 和 low\_model 链路各自有一个 UNETLoader，但**加载同一个模型文件**。ComfyUI 的模型缓存机制会自动复用，显存中只驻留一份模型，不会 OOM
...
- 示例: 241帧单次 → 3段×81帧拼接（每段末帧作为下段 start\_image）

**方向3: 降精度保帧数和分辨率**

- 优点: 保持时长和分辨率

- 缺点: 画质轻微下降，bf16 比 fp16\_fast 慢约 1.5 倍
...
- 示例: blocks\_to\_swap 20 → 24（递增 2-4 个单位）

- **注意**（C8 验证）：blocks\_to\_swap 值过高会导致专用显存闲置，转而使用共享 GPU 内存。L3 档推荐 20-24，非 36-38。
...
> **⚠️ 强制**：`sampler_name`（K 采样器）必须显式选择，根据提示词内容与目标效果决定，不可跳过或沿用默认值。**完整参考手册见独立章节 [4.19 K 采样器完全参考手册](#419-k-采样器完全参考手册comfyui-全家族)**（含全部采样器类别）。
...
| `rope_function`      | 高分辨率（480x848+）需 comfy\_chunked | comfy\_chunked           |
...
| 步骤 | 功能      | 对应函数                             | 说明                                                                    |
| -- | ------- | -------------------------------- | --------------------------------------------------------------------- |
| 1  | 模型查询展示  | `query_and_select_models()`      | `/object_info` API 查询 diffusion\_models/vae/clip/clip\_vision，按系列分组展示 |
...
| first\_last\_frame  | 首尾帧视频                   | WanFirstLastFrameToVideo（start\_image+end\_image）        |
| multi\_image\_video | 多图片生成视频（从多个图片中提取元素生成视频） | WanVideoClipVisionEncode（combine\_embeds=concat/average） |
| long\_video         | 长视频                     | ImageBatchExtendWithOverlap                              |
| video\_concat       | 视频拼接                    | VHS\_LoadVideo + ImageBatch                              |
| multi\_ref\_video   | 多参考视频                   | 多参考图嵌入                                                   |

**multi\_image\_video 多图视频生成要点（C5任务验证）**：
...
- 长视频超过单次帧数限制时采用分段生成+拼接，每段start\_image=前段末帧
...
| 使用 RealESRGAN\_x2 超分AI生成内容                            | 不适合AI生成内容，产生伪影                                                                              | 使用 2x\_StarSample\_V2.0 等 AI 专用放大模型                                                                 |
...
| 分段生成无末帧继承                                             | 段间转场突兀，角色不一致                                                                                | 提取前段末帧作为后段start\_image                                                                              |
| 视频任务只查 `images` 字段                                    | VHS\_VideoCombine 输出在 `gifs` 字段，漏检导致误判任务失败                                                  | 同时检查 `images` 和 `gifs` 字段                                                                           |
...
| Flux2 修正用英文提示词                                        | qwen\_3\_8b 对中文理解更好                                                                         | 使用中文提示词                                                                                             |
| PowerShell 文件复制                                       | 路径白名单权限静默失败                                                                                 | 用 Python shutil.copy2 + 验证文件大小                                                                      |
| KSamplerAdvanced widgets\_values 缺值                   | UI 格式按 position 顺序映射，缺 control\_after\_generate 导致全部错位                                      | 确保 widgets\_values 有 10 个值                                                                          |
| 多图视频仅靠 CLIP Vision concat                             | `combine_embeds="concat"` 仅合并语义特征向量，非像素合并，画面仍只有 start\_image 单图内容                           | 使用 `ImageConcatMulti` 水平拼接多图作为 `start_image`，CLIP Vision 仅作语义辅助引导                                   |
...
| 禁止                                   | 原因                                           | 正确做法                                   |
| ------------------------------------ | -------------------------------------------- | -------------------------------------- |
| 使用 KSampler 替代 WanVideoSampler       | 缺 motion\_scale/noise\_aug 控制，导致模糊噪点         | 使用 WanVideoSampler                     |
| 直接将 block\_swap\_args 传给 ModelLoader | LoRA 无法正确应用，导致旋转                             | 使用 WanVideoSetBlockSwap 独立节点           |
...
| noise\_aug\_strength=0               | 亮度锚定缺失                                       | noise\_aug\_strength=0.1               |
| 标准 I2V 模型启用 FLF2V 模式                 | FLF2V是专用模型功能，标准模型用fun\_or\_fl2v\_model=false | 仅FLF2V专用模型启用                           |
...
- 参考配置（L3高性能级）: 480x848 + 81-121帧 + 4-8步 + bf16 + BLOCKS\_TO\_SWAP=20-24（C8验证，非36-38）
...
- V18 首版 fp16\_fast + 480x848：OOM 崩溃
...
- 核心教训4：AI 图片放大用 2x\_StarSample\_V2.0，RealESRGAN\_x2 对 AI 内容产生伪影
...
- 核心教训4：段间过渡 motion\_latent\_count=0 + 修正图作为段2-5起点，避免暗→亮渐变

- 核心教训5：KSamplerAdvanced widgets\_values 必须 10 个值（含 control\_after\_generate），API格式按key映射、UI格式按position顺序
...
**功能**：创建 I2V 条件编码（concat\_latent\_image + concat\_mask）和空 latent，供 KSamplerAdvanced 采样。
...
1. `anchor_latent` = anchor\_samples 的 clone（形状 B,C,T,H,W，T=1）
2. `empty_latent` = zeros \[B, 16, ((length-1)//4)+1, H, W]（待采样的空 latent）
3. 若有 prev\_samples：`image_cond_latent` = concat(\[anchor\_latent, prev末尾motion\_latent\_count帧])，实现段间连贯
4. 填充 padding 至 total\_latents 长度
5. `mask` = ones，首帧=0（锚点固定不采样），其余=1（待生成）
6. 将 concat\_latent\_image 和 concat\_mask 写入 positive/negative 条件
...
| 节点                        | 源码位置                              | 功能                                                                                                   | 工作流参数                                                                         |
| ------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `PathchSageAttentionKJ`   | model\_optimization\_nodes.py:114 | 将模型注意力替换为 SageAttention（auto 自动选择最佳模式），加速注意力计算                                                       | sage\_attention="auto", allow\_compile=false                                  |
| `ModelPatchTorchSettings` | model\_optimization\_nodes.py:328 | 采样前启用 `torch.backends.cuda.matmul.allow_fp16_accumulation=True`，需 PyTorch 2.7.1+（本机 2.9.1+cu128 已满足） | enable\_fp16\_accumulation=true                                               |
| `LoraLoaderModelOnly`     | comfy-core                        | 加载 SVI HIGH lora（rank 128, fp16），修复长视频重复运动，提升动作连贯性                                                   | lora="SVI\_v2\_PRO\_...\_HIGH\_lora\_rank\_128\_fp16.safetensors", strength=1 |

**注意**：SageAttention 和 fp16\_accumulation 是显存和速度优化，**不改变生成质量**。SVI HIGH lora 才是质量增强的核心。三个节点分别应用于 high\_model 链路和 low\_model 链路（共 6 个节点实例）。
...
| 节点    | 模型文件                                                                 | 用途                            | 状态                |
| ----- | -------------------------------------------------------------------- | ----------------------------- | ----------------- |
| 节点2   | `Wan2.2_Remix_NSFW_i2v_14b_high_lighting_v2.0.safetensors`           | HIGH 链路 → SetNode high\_model | 启用                |
| 节点3   | `Wan2.2_Remix_NSFW_i2v_14b_high_lighting_v2.0.safetensors`（**同一文件**） | LOW 链路 → SetNode low\_model   | 启用                |
| 节点129 | `Wan2.2_2-I2V-A14B-LOW_fp8_e4m3fn_scaled_KJ.safetensors`             | 备用 LOW 模型                     | **未连接（links 为空）** |

**核心机制**：节点2 和节点3 加载**同一个模型文件**，ComfyUI 的模型缓存机制自动复用，显存中只驻留一份 14B 模型。high\_model 和 low\_model 虽名字不同，但指向同一模型实例。
...
- `steps2=2`（split\_step，INTConstant 节点31）
...
- 段1：WanImageToVideoSVIPro 的 prev\_samples 未连接（motion\_latent\_count=0）

- 段2-5：prev\_samples ← 前段 LOW KSamplerAdvanced 的输出 latent，motion\_latent\_count=1
...
- 逻辑：prefix(source\[:-5]) + linear\_blend(source\[-5:], new\[:5]) + suffix(new\[5:])
...
4. weight\_dtype 保持 "default"（让 ComfyUI 自动选择）
...
- **解决**：段 2-5 使用 Flux2 修正图作为 LoadImage 输入，保持 `motion_latent_count=0`，不连接 prev\_samples
...
| 特性        | API 格式                      | UI 格式                                |
| --------- | --------------------------- | ------------------------------------ |
| 参数映射      | 按 key 名映射（顺序无关）             | 按 widgets\_values position 顺序映射      |
| 节点结构      | `class_type` + `inputs`（字典） | `nodes`(含 widgets\_values) + `links` |
| 隐式 widget | 不需要                         | 必须包含（如 control\_after\_generate）     |

#### 4.17.2 KSamplerAdvanced widgets\_values 规范
...
**control\_after\_generate 可选值**：`"fixed"`, `"increment"`, `"decrement"`, `"randomize"`

**常见错误**：只有 9 个值（缺少 control\_after\_generate）→ 参数全部错位 → `Value not in list` / `Failed to convert`
...
| overlap\_mode  | 效果          | 适用场景          | 帧数损失        | 用户感知 |
| -------------- | ----------- | ------------- | ----------- | ---- |
...
| 参数             | 物理意义  | 推荐值                         | 梯度建议                                     |
| -------------- | ----- | --------------------------- | ---------------------------------------- |
| `overlap`      | 重叠帧数  | 1（cut 模式）或 5（linear\_blend） | 节点最小值为 1（不支持 0）；cut 模式下 overlap=1 等同直接拼接 |
...
### 4.19 K 采样器完全参考手册（ComfyUI 全家族）

> **本章节是采样器选择的唯一权威入口**：涵盖 ComfyUI 全部 K 采样器类别（Euler/Heun/DPM/DPM++/经典专业/res_multistep/LCM/DDPM），选型时在此浏览所有类别，按提示词内容与目标效果选择。
>
> **项目不强制绑定任何模型**：Wan2.2 只是本项目常用参考模型，其采样器约定见 [4.19.6 模型系列参考](#4196-模型系列参考wan22-仅作参考非强制)；其他模型系列请以模型官方文档 + 本手册通用方法论为准。

#### 4.19.1 预备知识：收敛型 vs 随机型 + 命名规则

**收敛型 vs 随机型（祖先型）**：

| 类型 | 核心机制 | 关键特性 | 适用场景 |
| :--- | :--- | :--- | :--- |
| **收敛型** | 每一步朝梯度计算"最优"方向去噪，路径固定 | 确定性高，同种子可复现，结构扎实 | 产品图、建筑效果图、商业工作流 |
| **随机型** | 每一步注入新随机噪声，路径随机游走 | 创造力强，纹理丰富，不收敛 | 概念艺术、风格化插画、角色设计 |

**命名规则解读**：

| 后缀/标记 | 含义 | 影响 |
| :--- | :--- | :--- |
| `ancestral` | 祖先采样器 | 每步注入噪声，随机型，不收敛 |
| `sde` | 随机微分方程 | 引入随机扩散，画面平滑自然 |
| `_gpu` | GPU 优化版本 | 利用 GPU 并行计算加速 |
| `_heun` | Heun 求解器 | 比默认 midpoint 精度更高 |
| `_cfg_pp` | CFG++ 增强 | 提示词遵循度更高 |
| `2s/2m/3m` | 求解器阶数（单步/二阶多步/三阶多步） | 阶数越高精度越高、越慢 |
| `pp2`/`bh2` | 特定算法修正 | 提高细节与一致性 |

#### 4.19.2 按提示词快速选型（决策表）

> **使用流程**：先看第 1 步判定收敛/随机，再看本决策表选采样器，最后查对应家族详解。

**第 1 步：判定画面目标 → 收敛型 or 随机型**

| 画面目标（从提示词判断） | 采样器类型 |
|------|-----------|
| 产品图、建筑效果图、商业工作流、需要可复现 | **收敛型** |
| 概念艺术、风格化插画、角色设计、创意探索 | **随机型** |

**第 2 步：按目标快速选型**

| 提示词/任务场景 | 首选采样器 | 调度器 | 步数 |
|------|-----------|--------|------|
| 日常通用/不知道选什么 | `dpmpp_2m` ★ | `karras` | 15-25 |
| 写实人像/产品摄影/平滑 | `dpmpp_2m_sde_gpu` | `karras` | 25-35 |
| 写实 + 极致细节 | `dpmpp_2m_sde_heun_gpu` | `karras` | 30-40 |
| 极致保真/大幅面打印 | `dpmpp_3m_sde_gpu` | `karras` | 30-50 |
| 创意/艺术感/笔触/动漫插画 | `euler_ancestral` 或 `dpmpp_2s_ancestral` | `karras` | 20-30 |
| 复杂长提示词/高控制力 | `dpmpp_2m_cfg_pp` | `beta` | 20-30 |
| 绝对结构稳定/线稿上色/老旧兼容 | `ddim` | `ddim_uniform` | 20-50 |
| 极速预览/批量测试 | `dpm_fast` 或 `res_multistep` | `karras` | 5-15 |
| 实时/极速交互（需 LCM LoRA） | `lcm` | `lcm` | 4-8 |
| SDXL 模型备选 | `uni_pc_bh2` | `karras` | 20-30 |
| 快速草稿/构图测试 | `euler` | `karras`/`normal` | 25-30 |
| 平滑渐变/柔和场景 | `dpmpp_sde_gpu` | `karras` | 25-35 |

#### 4.19.3 采样器家族详解（一）：Euler / Heun

**Euler 系列（速度与稳定基石）**：

| 采样器 | 类型 | 核心机制 | 特点 | 最佳场景 | 步数 | 调度器 |
|--------|------|---------|------|---------|------|--------|
| `euler` | 收敛 | 一阶欧拉方法，最简数值解法 | 干净偏朴素，速度快，细节精细度有限 | 快速草稿、图标/Logo、低重绘图生图(denoise<0.4) | 25-30 | `karras`/`normal` |
| `euler_ancestral` | 随机 | euler + 每步随机噪声 | 水彩/印象派纹理，不收敛，步数过多(>50)有噪点 | 艺术风格化、动漫/插画（吉卜力/新海诚风） | 20-30 | `karras` |
| `euler_cfg_pp` | 收敛 | euler + CFG++ 增强 | 保留速度+提升提示词控制 | 快速出图且需控制力 | 20-30 | `karras` |
| `euler_ancestral_cfg_pp` | 随机 | euler_ancestral + CFG++ | 创意与控制平衡 | 风格化创作需语义控制 | 20-30 | `karras` |

**Heun 系列（二阶精度进阶）**：

| 采样器 | 类型 | 核心机制 | 特点 | 最佳场景 | 步数 | 调度器 |
|--------|------|---------|------|---------|------|--------|
| `heun` | 收敛 | 二阶龙格-库塔(RK2)：预估+校正 | 更锐利、伪影少，光影过渡自然（约 2 倍 euler 耗时） | 写实人像（面部/皮肤）、风景摄影（云层/水面） | 20-25 | `karras` |
| `heunpp2` | 收敛 | heun 优化变体，调整预测-校正权重 | Heun 质量天花板，计算代价高 | 静物/珠宝/产品特写 | 20-30 | `karras` |

#### 4.19.4 采样器家族详解（二）：DPM / DPM++ 系列

> **现代工作流主力**。命名：`dpmpp` = DPM++ 算法家族，`2s/2m/3m` = 求解器阶数，`sde` = 随机微分方程，`ancestral` = 祖先采样，`_heun` = Heun 内部求解器，`_gpu` = GPU 加速，`_cfg_pp` = CFG++ 增强。

| 采样器 | 类型 | 核心机制 | 特点 | 最佳场景 | 步数 | 调度器 |
|--------|------|---------|------|---------|------|--------|
| `dpmpp_2m` ★ | 收敛 | DPM-Solver++(2M) 二阶多步，利用历史信息 | **行业黄金标准**：速度/质量/稳定性完美平衡 | **日常通用首选**（写实/二次元/商业） | 15-25 | `karras` |
| `dpmpp_2m_cfg_pp` | 收敛 | dpmpp_2m + CFG++ | 提示词遵循度高，保留 token 权重 | 复杂长提示词、商业约稿 | 20-30 | `beta` |
| `dpmpp_2m_sde` | 随机 | dpmpp_2m + SDE 随机扩散 | 极高平滑自然，"胶片感"，阴影/景深出色 | 写实人像特写、产品摄影、柔和氛围 | 25-35 | `karras` |
| `dpmpp_2m_sde_gpu` | 随机 | dpmpp_2m_sde GPU 加速 | 算法一致，更快 | 批量写真人像 | 25-35 | `karras` |
| `dpmpp_2m_sde_heun` | 随机 | dpmpp_2m_sde + Heun 内部求解器 | 比 midpoint 数值精度更高 | 极致细节、最终出图 | 30-40 | `karras` |
| `dpmpp_2m_sde_heun_gpu` | 随机 | 上述 + GPU 加速 | 算法一致，更快 | 极致细节 + 加速 | 30-40 | `karras` |
| `dpmpp_3m_sde` | 随机 | DPM-Solver++(3M) 三阶多步 + SDE | 理论最高保真度，步数越多边际收益越明显 | 大幅面打印、科研级图像、复杂纹理（毛皮/岩石） | 30-50 | `linear_quadratic`/`karras` |
| `dpmpp_3m_sde_gpu` | 随机 | dpmpp_3m_sde GPU 加速 | 算法一致，更快 | 高质量批量生成 | 30-50 | `karras` |
| `dpmpp_2s_ancestral` | 随机 | DPM-Solver++(2S) 二阶单步 + 祖先 | 纹理变化极大，"意外惊喜"，结构可能不稳 | 概念艺术、风格化插画、丰富笔触 | 20-30 | `karras` |
| `dpmpp_2s_ancestral_cfg_pp` | 随机 | 上述 + CFG++ | 创意与控制的平衡者 | 角色设计迭代、风格探索 | 20-30 | `karras` |
| `dpmpp_sde` | 随机 | 基于 SDE，模拟布朗随机扩散 | 极高平滑，"空气感/景深"，**显存占用较高** | 写实人像、产品摄影、柔和过渡 | 25-35 | `karras` |
| `dpmpp_sde_gpu` | 随机 | dpmpp_sde GPU 加速 | 算法一致，更快 | 批量生成 | 25-35 | `karras` |
| `dpm_2` | 收敛 | DPM 二阶求解器 | 介于 Euler 和 Heun 之间 | 备选 | 20-30 | `karras` |
| `dpm_2_ancestral` | 随机 | dpm_2 + 祖先采样 | 类似 euler a 加强版，控制力稍弱 | 创意备选 | 20-30 | `karras` |
| `dpm_fast` | - | 为速度优化的 DPM 变体 | 极快，少函数评估 | 低分辨率预览 | 5-15 | `karras` |
| `dpm_adaptive` | - | 自适应步长，按误差动态调整 | 质量极致，耗时不可预测 | 不计成本高质量 | 自动 | `karras` |

> **GPU 版本原则**：显卡性能足够（建议 RTX 20 系及以上）优先选 `_gpu` 版本——与无后缀版本算法完全一致，仅计算设备不同。

#### 4.19.5 采样器家族详解（三）：经典专业 / 特殊 / LCM

**经典与专业系列**：

| 采样器 | 类型 | 核心机制 | 特点 | 最佳场景 | 步数 | 调度器 |
|--------|------|---------|------|---------|------|--------|
| `ddim` | 收敛 | 去噪扩散隐式模型，非马尔可夫跳步采样 | 干净稳定，色彩饱和偏高，构图保持强，早期 ControlNet 标配 | 建筑线稿上色、绝对稳定、老旧工作流兼容 | 20-50 | `ddim_uniform` |
| `uni_pc` | 收敛 | 统一预测-校正(Unified Predictor-Corrector) | 效率高，社区支持度不如 DPM++ | 追求速度不牺牲太多质量 | 15-25 | `karras` |
| `uni_pc_bh2` | 收敛 | uni_pc 二阶修正版(BH2) | 细节与一致性更好，SDXL 兼容好 | SDXL 模型、dpmpp_2m 备选 | 20-30 | `karras` |
| `lms` | 收敛 | 线性多步法(Linear Multi-Step) | 老而稳，DPM++ 问世前常用 | 老旧工作流兼容（不推荐新项目） | 25-30 | `karras` |
| `deis` | 收敛 | 指数积分器采样器 | 较新，社区评估尚不充分 | 技术尝鲜、dpmpp_2m 实验替代 | 20-30 | `karras` |
| `ipndm`/`ipndm_v` | - | 改进伪数值方法，实验性 | 文档少，未充分验证 | 不建议生产，仅开发者测试 | - | - |
| `ddpm` | 收敛 | 原始马尔可夫链去噪（扩散"祖师爷"） | 极慢，精度一般 | 学术研究/对比实验/遗留兼容（不建议日常） | 100-1000 | `ddim_uniform` |

**特殊系列（Residual MultiStep & SEEDS）**：

| 采样器 | 类型 | 核心机制 | 特点 | 最佳场景 | 步数 | 调度器 |
|--------|------|---------|------|---------|------|--------|
| `res_multistep` | 收敛 | 分段优化策略，去噪过程智能分阶段 | 快，因模型而异，非通用最优 | NVIDIA Cosmos/MiniMax H3 特定模型、批量测试 | 15-25 | `karras` |
| `res_multistep_ancestral` | 随机 | 上述 + 祖先采样 | 快速创意 | 批量快速创意 | 15-25 | `karras` |
| `res_multistep_cfg_pp` | 收敛 | 上述 + CFG++ | 增强控制 | 批量测试 + 控制 | 15-25 | `karras` |
| `res_multistep_ancestral_cfg_pp` | 随机 | 全功能版（创意+控制） | 创意与控制兼备 | 综合场景 | 15-25 | `karras` |
| `seeds_2`/`seeds_3` | - | SEEDS 系列，内部机制未公开 | 实验性，社区使用少 | 仅开发者实验测试 | - | - |

**`lcm`（潜在一致性模型，非 K 采样器家族，但常见于列表）**：

| 采样器 | 核心机制 | 特点 | 最佳场景 | 步数 | 调度器 |
|--------|---------|------|---------|------|--------|
| `lcm` | 蒸馏技术，极少步数直达结果 | **必须配 LCM LoRA/专用模型**，否则效果极差；蒸馏风格，细节有损失 | 实时预览、视频流、移动端、极速交互 | 4-8 | `lcm` |

#### 4.19.6 模型系列参考（Wan2.2 仅作参考，非强制）

> **重要**：本项目的采样器选择是**一般性方法论**，不强制绑定任何模型。以下为各模型系列的采样器约定，其中 Wan2.2 是项目常用参考模型，其他系列按模型官方文档为准。

| 模型系列 | 常用采样器 | 调度器 | 说明 |
|---------|-----------|--------|------|
| **Wan2.2 视频（参考）** | `dpm++_sde` | `karras` | 本项目 +lightx2v 验证；unipc 导致动作卡住旋转（见 4.3.6 节） |
| Flux 1 | `euler` / `heun` | - | 见 4.4.2 节 |
| Flux 2 | `euler` | `simple` | cfg=1，见 4.4.2 节 |
| SD1.5/SDXL | `euler_ancestral` / `dpmpp_2m` | `karras`/`normal` | 见 4.4.1 节 |
| 其他系列 | 查模型官方文档 | - | 以官方推荐为准，本手册通用方法论兜底 |

#### 4.19.7 调度器协同（scheduler）

| 调度器 | 特点 | 推荐搭配 |
|--------|------|---------|
| `karras` | **最推荐**，Karras 噪声调度，质量更高 | 绝大多数采样器 |
| `normal` | 标准线性步长 | 通用 |
| `exponential` | 指数衰减，前期猛后期细 | SDE 类采样器 |
| `beta` | Beta 分布调度 | `dpmpp_2m_cfg_pp` |
| `linear_quadratic` | 线性二次调度 | `dpmpp_3m_sde` |
| `ddim_uniform` | DDIM 均匀调度 | `ddim`、`ddpm` |
| `sgm_uniform` | SGM 均匀调度 | 通用备选 |
| `lcm` | LCM 专用 | `lcm` |
| `simple` | 标准调度 | Flux2 + `euler` |

> **黄金法则**：无特殊说明时优先选 `karras` 调度器。

#### 4.19.8 按优先级选择

- **速度优先**：`dpmpp_2m` > `euler_ancestral` > `dpmpp_2m_sde_gpu`
- **质量优先**：`dpmpp_3m_sde_gpu` > `dpmpp_2m_sde_heun_gpu` > `dpmpp_2m_sde_gpu`
- **稳定性优先**：`dpmpp_2m` > `dpmpp_2m_cfg_pp` > `ddim`
- **创意优先**：`euler_ancestral` > `dpmpp_2s_ancestral` > `dpmpp_2m_sde_gpu`
...
1. 降低采样步数（30→20→15）
2. 增加 BlockSwap blocks\_to\_swap
3. 降低分辨率（保持 16 的倍数，可后续 X2 放大补偿）
4. 降精度（fp16\_fast → bf16）
5. 分段生成+拼接（保持总时长，禁止直接降帧数）
