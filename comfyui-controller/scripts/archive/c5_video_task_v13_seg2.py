"""v13段2重跑 (CLIP权重调整: 1.png=1.5, 末帧=0.5)"""
...
def build_seg_workflow(start_image_filename, positive_prompt, segment_name, use_end_image=False):
    """v13: CLIP concat权重调整 - 1.png=1.5(强约束角色), 末帧=0.5(弱场景)"""
    wf = {
        ...
        # v13关键修改: strength_1=1.5(1.png角色强约束), strength_2=0.5(末帧场景弱化)
        "clip_vision_encode": {"class_type": "WanVideoClipVisionEncode", "inputs": {
            ...
            "strength_1": 1.5,  # v13: 1.png高权重
            "strength_2": 0.5,  # v13: 末帧低权重
    ...
    wf["video_combine"] = {"class_type": "VHS_VideoCombine", "inputs": {
        ...
        "filename_prefix": f"c5_v13_{segment_name}_{int(time.time())}",
...
def main():
    print("=" * 60)
    print("v13段2+段3 (CLIP权重调整+段3提示词优化)")
    print(f"  CLIP: 1.png strength=1.5(强约束), 末帧 strength=0.5(弱化)")
    print(f"  段3: 详细描述从门口进入教室")
    print("=" * 60)

    # 段2
    print("\n[段2] 走向门口 (CLIP 1.png=1.5)")
    wf2 = build_seg_workflow(SEG1_LASTFRAME, POSITIVE_PROMPT_SEG2, "seg2", use_end_image=False)
    r2 = queue_prompt(wf2)
    ...
    # 提取段2末帧
    seg2_lf = os.path.join("E:/comfyui-cli/temp", f"c5_v13_seg2_lastframe_{int(time.time())}.png")
    ...
    print(f"  [请手动复制末帧到ComfyUI input后继续段3]")
    print(f"  源: {seg2_lf}")
    print(f"  目标: D:/2026-ComfyUI-V8.3/input/{seg2_lf_name}")
    print(f"\n  段2视频路径已保存: {seg2_video}")
    print(f"  段2末帧路径已保存: {seg2_lf}")
