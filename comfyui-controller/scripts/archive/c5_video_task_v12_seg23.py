"""v12段2+段3单独执行脚本 (段1已完成,末帧已复制到ComfyUI input)"""
...
SEG1_LASTFRAME = "c5_v12_seg1_lastframe_1784666164.png"
SEG1_VIDEO = "E:/comfyui-cli/output/c5_v12_seg1_1784665984_00001.mp4"
...
def build_workflow_seg(start_image_filename, positive_prompt, segment_name, use_end_image=False):
    ...
    wf["text_encode"] = {"class_type": "WanVideoTextEncode", "inputs": {"positive_prompt": positive_prompt, "negative_prompt": NEGATIVE_PROMPT, "t5": ["t5_loader", 0], "force_offload": True}}
...
def main():
    print("=" * 60)
    print("v12段2+段3执行 (段1已完成)")
    print(f"  段1末帧: {SEG1_LASTFRAME}")
    print("=" * 60)

    video_paths = [SEG1_VIDEO]

    # 段2
    print("\n[段2] 走向门口")
    wf2 = build_workflow_seg(SEG1_LASTFRAME, POSITIVE_PROMPT_SEG2, "seg2", use_end_image=False)
    r2 = queue_prompt(wf2)
    ...
    # 提取段2末帧
    seg2_lf = os.path.join("E:/comfyui-cli/temp", f"c5_v12_seg2_lastframe_{int(time.time())}.png")
    if not extract_last_frame(seg2_video, seg2_lf):
        print("段2末帧提取失败"); sys.exit(1)
    seg2_lf_name = os.path.basename(seg2_lf)
    # 复制到input (用PowerShell)
    copy_cmd = ["powershell", "-Command", f"Copy-Item '{seg2_lf}' 'D:/2026-ComfyUI-V8.3/input/{seg2_lf_name}' -Force"]
    cr = subprocess.run(copy_cmd, capture_output=True, text=True, timeout=30)
    if cr.returncode != 0 or not os.path.exists(f"D:/2026-ComfyUI-V8.3/input/{seg2_lf_name}"):
        print(f"段2末帧复制失败: {cr.stderr}"); sys.exit(1)
    print(f"  段2末帧: {seg2_lf_name}")

    # 段3
    print("\n[段3] 进入教室→拉椅→落座")
    wf3 = build_workflow_seg(seg2_lf_name, POSITIVE_PROMPT_SEG3, "seg3", use_end_image=True)
    r3 = queue_prompt(wf3)
    pid3 = r3.get("prompt_id")
    print(f"  prompt_id: {pid3}")
    seg3_video = wait_video(pid3, "段3")
    if not seg3_video:
    ...
    if r.returncode != 0:
        cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_file,
               "-c:v", "libx264", "-crf", "14", "-pix_fmt", "yuv420p10le", "-r", "24", merged_path]
    ...
    if os.path.exists(merged_path):
        ...
        for i, vp in enumerate(video_paths, 1):
            print(f"  段{i}: {vp} ({os.path.getsize(vp)/1048576:.2f}MB)")
        print(f"  最终(10秒): {merged_path} ({size:.2f}MB)")
        print("=" * 60)
    else:
        print("拼接失败")
