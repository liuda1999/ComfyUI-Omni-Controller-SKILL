"""
C3视频生成任务 - V19架构
任务：使用c3_1.png生成10秒@24fps女仆舞蹈视频
动作：起身→旋转→垫步摆裙→举臂转体→碎步手势→行礼收尾
规格：10秒，24fps，480x848，241帧
策略：单次生成（V19验证架构），固定相机为主，动作简化为关键节点
"""
...
# 提示词（三段式结构 + 简化关键动作节点 + 固定相机为主）
POSITIVE_PROMPT = (
    "masterpiece, best quality, 8k, highly detailed, fixed camera, medium full body shot, indoor scene, "
    "subject appearance clear, same person as reference image, maid outfit with lace and ribbon, "
    "consistent appearance throughout the video, "
    "the person slowly stands up from a crouching position, "
    "then performs a graceful 360 degree clockwise spin in place, "
    "then does small side steps while gently swaying the skirt, "
    "then raises both arms in a wave motion and does a small turn, "
    "then does small forward and back steps while making a heart gesture with right hand, "
    "finally stops and does a polite maid curtsy bow with both hands lifting the skirt, "
    "smooth body motion, natural continuous movement, elegant and graceful, "
    "no turning around, no spinning, standing in place, fixed camera, no camera movement"
)

NEGATIVE_PROMPT = (
    "面部崩坏，五官错位，表情扭曲，肢体畸形，多手多脚，手指异常，服装穿模，裙摆穿帮，"
    "画面闪烁，跳帧卡顿，人物漂移，场景错乱，背景突变，模糊重影，比例失调，违和动作，"
    "低画质，噪点，脏污，运镜剧烈晃动，人物出框, "
    ...
    workflow = {
        ...
        "video_combine": {
            ...
            "inputs": {
                ...
                "frame_rate": 24,
                "loop_count": 0,
                "filename_prefix": f"c3_task_{int(time.time())}",
...
def main():
    print("=" * 60)
    print("C3视频生成任务 - V19架构 (10秒@24fps)")
    ...
    print("动作: 起身→旋转→垫步摆裙→举臂转体→碎步手势→行礼收尾")
    print("=" * 60)
