...
def _get_embedded_python_binary(workspace_path: str) -> str | None:
    """Locate ComfyUI's bundled embedded Python (official distribution layout).

    Windows distributions ship python.exe directly under `python/` or
    `python_embeded/` (no Scripts/ subdirectory). This mirrors the lookup
    order used by scripts/start_server.py so the CLI picks the same
    interpreter that the skill-mode scripts use.
    """
    if platform.system() == "Windows":
        for name in ("python", "python_embeded"):
            candidate = os.path.join(workspace_path, name, "python.exe")
            if os.path.isfile(candidate):
                return candidate
    else:
        for name in ("python", "python_embeded"):
            candidate = os.path.join(workspace_path, name, "bin", "python")
            if os.path.isfile(candidate):
                return candidate
    return None
...
def resolve_workspace_python(workspace_path: str | None = None) -> str:
    ...
    if workspace_path is not None:
        # Prefer ComfyUI's bundled embedded Python (the interpreter that
        # actually has torch/CUDA installed). A leftover `venv/` from a
        # previous Administrator install can be broken (points at a missing
        # system Python), so we check the embedded layout first.
        embedded = _get_embedded_python_binary(workspace_path)
        if embedded:
            return embedded
...
def ensure_workspace_python(workspace_path: str) -> str:
    ...
    # Prefer ComfyUI's bundled embedded Python (matches resolve_workspace_python).
    embedded = _get_embedded_python_binary(workspace_path)
    if embedded:
        return embedded
