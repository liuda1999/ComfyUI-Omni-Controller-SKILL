...
from cli import constants, logging_utils, ui
...
def track_event(event_name: str, properties: Any = None, *, mixpanel_name: str | None = None):
    ...
    if properties is None:
        properties = {}
    logging_utils.debug(f"tracking event called with event_name: {event_name} and properties: {properties}")
    ...
    for provider in PROVIDERS:
        ...
        try:
            ...
            logging_utils.warning(f"Failed to track event via {type(provider).__name__}: {e}")
...
def track_command(sub_command: str = None):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            logging_utils.debug(f"Tracking command: {command_name} with arguments: {filtered_kwargs}")
...
def init_tracking(enable_tracking: bool):
    ...
    """
    global user_id
    logging_utils.debug(f"Initializing tracking with enable_tracking: {enable_tracking}")
    ...
    logging_utils.debug(f'User identifier for tracking user_id found: {curr_user_id}."')
    if curr_user_id is None:
        curr_user_id = str(uuid.uuid4())
        config_manager.set(constants.CONFIG_KEY_USER_ID, curr_user_id)
        logging_utils.debug(f'Setting user identifier for tracking user_id: {curr_user_id}."')
    ...
    if not install_event_triggered:
        logging_utils.debug("Tracking install event.")
...
def _flush_all_providers() -> None:
    for provider in PROVIDERS:
        try:
            ...
            logging_utils.warning(f"Failed to flush telemetry provider {type(provider).__name__}: {e}")
