...
from cli.command.custom_nodes.cm_cli_util import execute_cm_cli
...
class BisectState(NamedTuple):
    ...
    def good(self) -> BisectState:
        ...
        # 保留原始顺序：bisect 二分法依赖确定的顺序，set() 运算会打乱顺序
        active_set = set(self.active)
        new_range = [n for n in self.range if n not in active_set]
    ...
    def inactive_nodes(self) -> list[str]:
        # 保留 self.all 的原始顺序
        active_set = set(self.active)
        return [n for n in self.all if n not in active_set]
...
def start(
    ...
    if not nodes_list:
        typer.echo("No enabled custom nodes found to bisect.")
        raise typer.Exit()
    state = BisectState(
        status="running",
        all=nodes_list,
