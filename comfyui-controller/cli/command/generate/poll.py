...
from cli.command.generate import client
    ...
    while _now() < deadline:
        ...
        if on_progress is not None:
            ...
            if isinstance(progress, int | float):
                # Normalize: some partners report 0-100, others 0-1.
                # poll_generic applies the same heuristic.
                p = float(progress)
                on_progress(p / 100.0 if p > 1 else p)
        if status == "Ready":
