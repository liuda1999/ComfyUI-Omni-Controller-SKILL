...
from cli import tracking, ui
from cli.command.generate import adapters, client, output, poll, schema, spec, upload
...
def _emit_result(result: poll.PollResult, *, request_id: str, download: str | None, as_json: bool) -> None:
    if as_json:
        ...
        # In --json mode, still exit non-zero on failure so scripts/agents
        # can detect the failure via exit code. The raw JSON already carries
        # the status field; the non-zero exit is the signal to stop.
...
def _list_models(extra_args: list[str]) -> None:
    ...
    # Handle --help/-h explicitly: the generate command uses
    # help_option_names=[] so typer won't auto-intercept these flags.
    if any(a in {"--help", "-h"} for a in extra_args):
        rprint("[bold]Usage:[/bold] comfy generate list [OPTIONS]")
        rprint("")
        rprint("Show available partner models with their short aliases.")
        rprint("")
        rprint("[bold]Options:[/bold]")
        rprint("  --partner <name>   Filter by partner (e.g. bfl, ideogram, openai, recraft)")
        rprint("  --category <name>  Filter by category/style (e.g. text-to-image, image-edit)")
        rprint("  --query <text>     Fuzzy-filter model names/summaries")
        rprint("  --help             Show this message and exit")
        raise typer.Exit(code=0)
...
def _resume(extra_args: list[str]) -> None:
    ...
    try:
        ...
        rprint(f"[bold red]--timeout: expected number, got {meta.get('timeout')!r}[/bold red]")
        raise typer.Exit(code=1)
    download = meta.get("download") if isinstance(meta.get("download"), str) else None
    as_json = bool(meta.get("json", False))
    ...
    with _spinner() as prog:
        ...
        try:
            ...
            rprint(f"[bold red]Polling failed: {e}[/bold red]")
            raise typer.Exit(code=1) from e
    _emit_result(result, request_id=job_id, download=download, as_json=as_json)
